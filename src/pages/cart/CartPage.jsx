...
const CartPage = ({ cartOpen }) => {
	...
	return (
		...
		products.length !== 0 && (
			<div
				className="absolute w-96 top-[106px] h-[585px] right-0 z-40 bg-gray-400  ease-in-out duration-200"

				// className={`${
				// 	cartOpen ? "absolute w-96 opacity-1 top-[110px] right-0 " : "w-0 opacity-0 "
			...
			>
				...
				<div className="flex flex-col justify-end p-4">
					<Typography>total Product:{products.length} </Typography>
					<Typography>
						total Price:<span className="text-lg font-bold text-green-500">{totalCount}</span>
					</Typography>
				</div>
			</div>
		)
